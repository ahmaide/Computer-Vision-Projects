...
def plot_all_queries(results, dataset, index):
    num_columns = 6
    num_rows = 2
    for i in range(0, len(results), num_rows):
        ...
        plt.suptitle(f"CBIR Color Moments Results - weights: {index}", fontsize=16)
...
def plot_ROC(roc_auc, index, fprs, tprs):
    plt.figure()
    plt.plot(fprs, tprs, color='darkorange', lw=2, label=f'ROC Curve (area = {roc_auc:.2f}) for weights {index}')
    plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title(f'Receiver Operating Characteristic for Color Moments with Weights {index}')
...
def print_results(results, index, threshold):
    ...
    print(f"For the weights {index}, with threshold {threshold}:")
